...
public abstract class LoadXML
{
    ...
    {
        string text = LoadXML.ReadContent(node).ToLower();
        if (text == "true")
        {
            return true;
        }
    ...
    public static T ReadObject<T>(XmlNode node) where T : LoadXML
    {
    ...
    public static T ReadDynamicObject<T>(XmlNode node) where T : LoadXML
    {
        string typeName = LoadXML.ReadAttribute(node, TYPENAME);
    ...
    public static void WriteDynamicObject(TextWriter os, string name, LoadXML x)
    {
    ...
    public static void WriteDynamicObject<T>(TextWriter os, string name, List<T> x) where T : LoadXML
    {
        os.WriteLine("<{0}>", name);
        x.ForEach(delegate(T v)
        {
            LoadXML.WriteDynamicObject(os, NODENAME, v);
        });
        os.WriteLine("</{0}>", name);
    }
    ...
    public static void Write(TextWriter os, string name, LoadXML x)
    {
        if (x == null)
        {
            return;
    ...
    public static void Write<V>(TextWriter os, string name, HashSet<V> x)
    {
        ...
        {
            LoadXML.Write(os, NODENAME, current);
        }
        os.WriteLine("</{0}>", name);
    }
    ...
    public static void Write<K, V>(TextWriter os, KeyValuePair<K, V> x)
    {
        ...
        if (x.Value is LoadXML)
        {
            LoadXML v = x.Value as LoadXML;
    ...
    public static void Write(TextWriter os, string name, object x)
    {
        ...
        if (x is bool)
        {
            LoadXML.Write(os, name, (bool)x);
        }
        else
        {
            if (x is int)
            {
                LoadXML.Write(os, name, (int)x);
            }
            else
            {
                if (x is long)
                {
                    LoadXML.Write(os, name, (long)x);
                }
                else
                {
                    if (x is float)
                    {
                        LoadXML.Write(os, name, (float)x);
                    }
                    else
                    {
                        if (x is string)
                        {
                            LoadXML.Write(os, name, (string)x);
                        }
                        else
                        {
                            if (x is Vector3)
                            {
                                LoadXML.Write(os, name, (Vector3)x);
                            }
                            else
                            {
                                if (!(x is LoadXML))
                                ...
                                LoadXML.Write(os, name, (LoadXML)x);
                            }
                        }
                    }
                }
    ...
    public void Load(string file)
    {
        ...
        TextAsset pAsset = LoadResource.Instance.Load<TextAsset>(file);
        if (pAsset == null)
        {
            return;
        }
    ...
    public void Save(string file)
    {
        ...
        LoadXML.Write(stringWriter, "Root", this);
        File.WriteAllText(file, stringWriter.ToString());
    }

    public static V LoadSingleConfig<V>(string file) where V : LoadXML
    {
        List<V> list = new List<V>();
        LoadXML.LoadConfig<V>(list, file);
        if (list.Count != 1)
        {
    ...
    public static void SaveSingleConfig<V>(V x, string file) where V : LoadXML
    {
        LoadXML.SaveConfig<V>(new List<V>
            {
                x
            }, file);
    }

    public static void LoadConfig<V>(List<V> x, string file) where V : LoadXML
    {
        XmlDocument xmlDocument = new XmlDocument();
        TextAsset pAsset = LoadResource.Instance.Load<TextAsset>(file);
        if (pAsset == null)
        ...
        xmlDocument.LoadXml(pAsset.text);
        foreach (XmlNode current in LoadXML.GetChilds(xmlDocument.DocumentElement))
        {
            V data = LoadXML.ReadObject<V>(current);
            x.Add(data);
        }
    }

    public static void SaveConfig<V>(List<V> x, string file) where V : LoadXML
    {
        StringWriter stringWriter = new StringWriter();
        LoadXML.Write<V>(stringWriter, "Root", x);
        File.WriteAllText(file, stringWriter.ToString());
    }

    public static void SaveConfig<K, V>(Dictionary<K, V> x, string file) where V : LoadXML
    {
        LoadXML.SaveConfig<V>(x.Values.ToList<V>(), file);
    }
